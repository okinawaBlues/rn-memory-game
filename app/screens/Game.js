...
export default class Game extends Component {
  ...
  componentDidMount() {
    ...
    Alert.alert("Welcome to the easy level of the game",
    "Your goal is to find all the pairs of each icon on the screen. Tap the (?) and to open them and see their content to match all the pairs!");
  }
    ...
    if ( cards[index].is_open == false && selected_pairs.indexOf(cards[index].name) == -1 ) {
      ...
      if (current_selection.length == 2) {
        if (current_selection[0].name == current_selection[1].name) { //checking if names of 2 selected card objects have the same name attribute
          ...
          if (score == 8) { // checking if all pairs have been opened
            Alert.alert( "Awesome!",
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.resetButton}>
          ...
          <Button onPress={() => this.props.navigation.push("Game2")} title="Next level" color="#87499d" />
        </View>
      </View>
    );
  }
